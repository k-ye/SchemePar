...
def SchemeParser():
  def p_program(p):
    ...
    '''program : expr
               | LPAREN func_define_list RPAREN expr'''
    func_define_list, body = [], None
    if len(p) > 2:
        func_define_list = p[2]
        body = p[4]
    else:
        func_define_list = []
        body = p[1]

    p[0] = MakeSchProgramNode(func_define_list, body)
  ...
  def p_expr_lambda(p):
    # expr = (lambda (maybe_func_params_list) body)
    'expr : LPAREN LAMBDA LPAREN maybe_func_params_list RPAREN expr RPAREN'
    params, body = p[4], p[6]
    p[0] = MakeSchLambdaNode(params, body)

  def p_func_define(p):
    # func_define = (define func_signature body)
    'func_define : LPAREN DEFINE func_signature expr RPAREN'
    name, params = p[3]
    body = p[4]
    p[0] = MakeSchFuncDefineNode(name, params, body)

  def p_func_signature(p):
    # the first arg_var is the function name
    '''func_signature : arg_var
                      | LPAREN arg_var maybe_func_params_list RPAREN'''
    if len(p) > 2:
      p[0] = (p[2], p[3])
    else:
      p[0] = (p[1], [])  # empty function parameters

  def p_maybe_func_params_list(p):
    '''maybe_func_params_list : empty
                              | arg_var maybe_func_params_list'''
    p[0] = []
    if len(p) > 2:
      p[0] = [p[1]] + p[2]  # non-empty case

  def p_func_define_list(p):
    '''func_define_list : func_define 
                        | func_define func_define_list'''
    p[0] = [p[1]]
    if len(p) > 2:
      p[0] = p[0] + p[2]
